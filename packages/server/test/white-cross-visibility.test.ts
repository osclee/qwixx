...
describe("white-phase cross visibility", () => {
  ...
  it("shows a player's white cross immediately, before other players (including the active one) have answered", async () => {
    ...
    const server = await startTestServer({ seed: 6, whitePhaseMs: 60_000, colorPhaseMs: 60_000 });
    close = server.close;
    ...
    const passivePlayerId = passive === alice ? aliceJoined.playerId : bobJoined.playerId;
    ...
    passive.send({ type: "submit_white", action: { kind: "cross", color: "red", value: sumWhite } });
    ...
    const midWaitSnap = await alice.waitForSnapshot((s) => s.whiteSubmitted.includes(passivePlayerId));
    expect(midWaitSnap.phase).toBe("WHITE"); // still waiting on the active player
    const passiveSheet = midWaitSnap.sheets.find((s: any) => s.playerId === passivePlayerId);
