...
export interface SerializedGameState extends Omit<GameState, "diceInPlay" | "removedColors"> {
  diceInPlay: Color[];
  removedColors: Color[];
}
...
export function deserializeGameState(serialized: SerializedGameState): GameState {
  return {
    ...serialized,
    diceInPlay: new Set(serialized.diceInPlay.filter((c) => COLORS.includes(c))),
    removedColors: new Set(serialized.removedColors.filter((c) => COLORS.includes(c))),
  };
}
